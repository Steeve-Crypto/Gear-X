...
## Completion status
Complete for automated scope: 19 Node domain/SQLite/static tests and 27 Jest React Native/provider/runtime/pipeline tests are invoked by validation, including capability selection, native speech events, fallback, timeout, consent, malformed output, actual multipart upload, legacy mode migration, and cloud budget behavior. The physical-device matrix remains a release gate in task 014.
